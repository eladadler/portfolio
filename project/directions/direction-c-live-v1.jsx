...
// JS character-decode for Scramble (fits the mono "studio sheet" aesthetic).
function FxHeading({ text, fx }) {
  ...
  const runScramble = () => {
    ...
    const tick = (t) => {
      ...
      for (let i = 0; i < final.length; i++) {
        ...
        out += i < reveal ? ch : chars[(Math.random() * chars.length) | 0];
      }
      el.textContent = out;
  ...
  if (fx === "Scramble") {
    return <span ref={ref} className="fx-heading fx-scramble" onMouseEnter={runScramble}>{text}</span>;
  }
  const cls = fx && fx !== "None" ? "fx-heading fx-" + fx.toLowerCase() : "fx-heading";
...
function CProjectsSpread({ alternate = true, headingFx }) {
  const D = window.PORTFOLIO_DATA;
  return (
      ...
      {D.projects.map((p, i) => (
        <article className={"c-spread" + (alternate && i % 2 === 1 ? " flip" : "")} key={p.num}>
          <div className="imgside">
            <PhotoSlot label={p.placeholder} style={{ height: "auto", flex: 1, border: "none" }} />
          </div>
...
function DirectionCLive() {
  ...
  const rootStyle = {
    ...
    "--accent": theme.accent, "--accent-rgb": theme.accentRgb, "--accent-bright": theme.accentBright,
  ...
  return (
    <div className="dirC" style={rootStyle}>
      ...
      <CDotField active={tweaks.texture === "Cursor"} inkRgb={theme.inkRgb} accentRgb={theme.accentRgb} reduce={reduce} />
      ...
      {tweaks.projectsLayout === "Spreads" ? (
        <CProjectsSpread alternate={tweaks.alternateSides} headingFx={tweaks.headingFx} />
      ) : (
